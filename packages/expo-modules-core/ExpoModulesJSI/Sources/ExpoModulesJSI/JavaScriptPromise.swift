...
public struct JavaScriptPromise: JavaScriptType, ~Copyable {
  ...
  public init(_ runtime: JavaScriptRuntime) {
    ...
    // Spawn a detached unstructured task that completes when the promise is resolved/rejected.
    // This is useful to "await" on the promise on the native side (see `get` function).
  ...
  func resolve(_ result: consuming JavaScriptValue) {
    ...
    runtime.execute { [data, resolveFunction] in
  ...
  func reject(_ error: any Error) {
    ...
    // `JavaScriptActor` does not guarantee thread safety here. Make sure to jump to JS thread.
    runtime.execute { [data, rejectFunction] in
