...
open class JavaScriptRuntime: Equatable, @unchecked Sendable {
  ...
  /**
   Creates a runtime from the raw pointer to JSI runtime.
   */
...
//    self.scheduler = expo.RuntimeScheduler(pointee)
   ...
   */
  @JavaScriptActor
  public func createAsyncFunction(_ name: String, @_inheritActorContext _ function: sending @escaping AsyncFunctionClosure) -> JavaScriptFunction {
  ...
  public func assertThread(file: String = #file, function: String = #function, line: Int = #line) {
    precondition(isOnJavaScriptThread(), "Function '\(function)' is not run on the JavaScript thread (\(file):\(line))")
  }
