...
it('dispatches only one action when re-tapping active tab with nested stack', async () => {
  ...
  // CRITICAL: Should only dispatch ONE action, not two
  // Bug: Currently dispatches JUMP_TO then POP_TO_TOP
  expect(dispatchedActions).toHaveLength(1);

  // Verify it's a JUMP_TO action (not followed by POP_TO_TOP)
